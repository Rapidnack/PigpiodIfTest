	...
	public class PigpiodIf : IDisposable
	{
		...
		public int serial_open(string dev, UInt32 baud, UInt32 flags)
		{
			...
			*/
			...
			var bytes = System.Text.Encoding.UTF8.GetBytes(dev);
		...
		int shell_(string scriptName, string scriptString)
		{
			...
			*/
			...
			exts[0].Contents = System.Text.Encoding.UTF8.GetBytes(scriptName + "\0"); /* include null byte */

			exts[1].Contents = System.Text.Encoding.UTF8.GetBytes(scriptString);
		...
		int file_open(string file, UInt32 mode)
		{
			...
			*/
			...
			exts[0].Contents = System.Text.Encoding.UTF8.GetBytes(file);
		...
		int file_list(string fpat, byte[] buf)
		{
			...
			*/
			...
			exts[0].Contents = System.Text.Encoding.UTF8.GetBytes(fpat);
